...
namespace nav2_custom_controller {
  ...
  node_->get_parameter(plugin_name_ + ".max_angular_speed", max_angular_speed_);
    ...
    void CustomController::cleanup() {
        RCLCPP_INFO(node_->get_logger(), " Cleaning up controller: %s type is nav2_custom_controller::CustomController", plugin_name_.c_str());
    }
    void CustomController::activate() {
        RCLCPP_INFO(node_->get_logger(), " Activating controller: %s type is nav2_custom_controller::CustomController", plugin_name_.c_str());
    }
    void CustomController::deactivate() {
        RCLCPP_INFO(node_->get_logger(), " Deactivating controller: %s type is nav2_custom_controller::CustomController", plugin_name_.c_str());
    }
    geometry_msgs::msg::TwistStamped CustomController::computeVelocityCommands(const geometry_msgs::msg::PoseStamped &pose, const geometry_msgs::msg::Twist &, nav2_core::GoalChecker *){
    // 1.检查路径是否为空
    if (global_plan_.poses.empty()) {
        throw nav2_core::PlannerException("Received empty path");
    }
    ...
    RCLCPP_INFO(node_->get_logger(), "Controller: %s sending velocity (%f,%f)", plugin_name_.c_str(), cmd_vel.twist.linear.x, cmd_vel.twist.angular.z);
    return cmd_vel;
    }
    ...
    geometry_msgs::msg::PoseStamped CustomController::getNearestTargetPose(const geometry_msgs::msg::PoseStamped &current_pose) {
        // 1.遍历路径获取路径中距离当前点最近的点的索引，存储到nearest_pose_index
