...
public:
    ...
    if (response == nullptr) {
      ...
      for (auto result : response->results) {
        if (result.successful) {
          ...
          RCLCPP_WARN(this->get_logger(), " 参 数 k 失 败 原 因：%s", result.reason.c_str());
        }
      }
    }
  }
  void timer_callback() {
    ...
    while (!patrol_client_->wait_for_service(std::chrono::seconds(1))){
        ...
        RCLCPP_INFO(this->get_logger(), " 服务端还没上线，请等待 ...");
    }
    // 2.构造请求的对象
    ...
    patrol_client_->async_send_request(
        request,
        [&](rclcpp::Client<Patrol>::SharedFuture result_future){
...
int main(int argc, char **argv) {
  ...
  node->update_server_param_k(1.5); // 修改服务器参数k为1.5
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}