...
- 3.**simulation**
  ...
  - `python livox_teleop.py`（启动键盘控制）
  
    - `rviz_config_dir`和`map_yaml_path`和`pcd_file_path`三个路径需要修改
    - 初始化位置修改
    - `get_init_pose.py`（获取`2D Pose Estimate`的位置信息）（rviz）（在`navigation2.launch.py`里面修改）
    - 使用`gazebo`左侧栏目移动以及获取位置信息（gazebo）（在`gazebo_sim.launch.py`里面修改）
